...
import { getCMGuideBySlug, getAllCMGuideSlugs } from "@/lib/guides";
...
export async function generateStaticParams() {
  const slugs = getAllCMGuideSlugs();
  return slugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const guide = getCMGuideBySlug(slug);

  if (!guide) {
    return {
      title: "Guide Not Found | umaAssist",
    };
  }

  return {
    title: `${guide.title} | umaAssist`,
                  ...
                  <div className="mb-5 mr-10 flex items-center">
                    <div className="mr-4">
                      <div className="relative h-10 w-10 overflow-hidden rounded-full">
                        <Image src={guide.author.image} alt={guide.author.name} fill />
                      </div>
                    </div>
